        ...
        if f.metadata().unwrap().is_file() {
            ...
            for (i, line) in content.lines().enumerate() {
                ...
                if let Some(m) = m {
                    ...
                    if i +1 < content.lines().count() && content.lines().nth(i + 1).unwrap().contains("pallet::call_index") {
                        continue;
                    }

                    println!("{}:{} index {}", f.path().display(), i, call_index);
                    new_lines.push(format!("{}#[pallet::call_index({})]", m.get(1).unwrap().as_str(), call_index));
                    call_index += 1;
                }
